...
# Simple dummy object used in placement tests
@dataclass
class Obj:
    id: str
    x: float
...
# Minimal layout model for testing placement logic
class DummyLayout:
...
def test_collides_touching_edge_not():
    # Touching edge is NOT collision in this implementation
...
def test_can_place_outside_room():
    ...
    ok, msg = can_place_object(layout, Rect(95, 95, 20, 20))
    assert not ok
...
def test_can_place_collision():
    ...
    ok, msg = can_place_object(layout, Rect(15, 15, 20, 20))
    assert not ok
...
def test_can_place_too_close():
    ...
    ok, msg = can_place_object(layout, Rect(25, 10, 10, 10))
    assert not ok
...
def test_can_place_valid():
    ...
    ok, msg = can_place_object(layout, Rect(30, 10, 10, 10))
    assert ok
...
def test_move_object_success():
    ...
    ok, msg = move_object(layout, "a", 50, 50)
    assert ok
...
def test_move_object_collision():
    ...
    ok, msg = move_object(layout, "a", 38, 10)
    assert not ok
...
def test_move_object_id_not_found():
    ...
    assert not ok
    # KeyError message looks like "'missing'"
